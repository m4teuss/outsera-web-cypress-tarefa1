...
Then("devo ser redirecionado para tela principal", () => {
  cy.url().should("include", "https://practice.qabrains.com/?logged=true");
});

And("devo visualizar a seguinte mensagem {}", (mensagemSucesso) => {
  const seletores = [".success-msg", ".title"];

  seletores.forEach((seletor) => {
    cy.get(seletor).should("be.visible").and("contain", mensagemSucesso);
  });
});
